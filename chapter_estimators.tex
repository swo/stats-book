...
\expect{\hat{\mathbb{V}}_X}
  ...
  &= \frac{\var{X}}{n} (n - 1) \quad{\text{(using those \\
  &= \frac{n-1}{n} \var{X}.
\end{align*}
where I chose $Z_1$ arbitrarily. All the $Z_i$ are identically distributed, so
I just picked that one.

\hl{consistency?}

Let's check that this estimator meets some of our standard criteria. Is it
unbiased?
\begin{align*}
